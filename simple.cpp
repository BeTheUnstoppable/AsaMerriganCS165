...
int main(int argc, char *argv[])
{
	...
	char infilename[] = "merrigan.txt";
	char outfilename[] = "output.txt";

	unsigned char buffer[1024]={0};
	unsigned char buffer0[1024]={0};
	...
	BIO *rsaprikey;
	rsaprikey = BIO_new_file("rsaprivatekey.pem", "r");
	RSA *rsa0;
	rsa0 = PEM_read_bio_RSAPrivateKey(rsaprikey,NULL,0,NULL);

	BIO *rsapubkey;
	rsapubkey = BIO_new_file("rsapublickey.pem", "r");
	RSA *rsa1;
	rsa1 = PEM_read_bio_RSA_PUBKEY(rsapubkey,NULL,0,NULL);
	...
	RSA_private_encrypt( 20, buffer,buffer0, rsa0, RSA_PKCS1_PADDING);

	RSA_public_encrypt( 128, buffer0,buffer1, rsa1, RSA_PKCS1_PADDING);
	
	for(int i = 0; i < 128; i++)
	{
		//Print two hexadecimal digits (8 bits or 1 character) at a time
		printf("%02x", buffer1[i] & 0xFF);
	}
	printf("\n");
